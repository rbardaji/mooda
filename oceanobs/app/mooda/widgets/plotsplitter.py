...
class PlotSplitter(QSplitter):
    ...
    def initUI(self):
        ...
        self.dropWidget.list2drop[list, list].connect(self.dropData)
    ...
    def dataListClick(self):
        if not self.autoPlotRadioButton.isChecked():
            return
    ...
    def addData(self, data):
        ...
        # Clear the list
        self.dataList.clear()
        # Just add keys without QC
        key_no_qc = [key for key in data.keys() if '_QC' not in key
                     if not is_acoustic_data(key)]
        self.dataList.addItems(
        ...
        for i in range(self.dataList.count()):
            if "_QC" in self.dataList.item(i).text():
                ...
                self.dataList.item(i).setToolTip('{} ({})'.format(
                    self.wf.meaning[self.dataList.item(i).text()]['long_name'],
                    self.wf.meaning[self.dataList.item(i).text()]['units']))
                msg += "\n- {}: {} ({})".format(
        ...
        self.dropWidget.addLabels(key_no_qc)
        self.qcWidget.addLabels(key_no_qc)
        self.renameWidget.addLabels(key_no_qc)
    ...
    def dropData(self, labels, flagList):
        """
