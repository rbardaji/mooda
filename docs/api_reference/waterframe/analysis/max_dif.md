...
* parameter1: Key name of column 1 to calculate the difference. (str)
* parameter2: Key name of column 2 to calculate the difference. (str)

### Returns

* where: The position (index) of WaterFrame.data. (Index)
* value: Value of the maximum difference. (float)
...
indexes, value = wf.max_diff('TEMP', 'PSAL')
# In this case, the dataset contains 3 indexes: DEPTH, POSITION and TIME
depth, position, time = indexes

print(f"The maximum diference between {wf.vocabulary.get(parameter1).get('long_name')} "
      f"and {wf.vocabulary.get(parameter2).get('long_name')} is {value}, at {time}")
```
